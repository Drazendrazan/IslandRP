...
Citizen.CreateThread(function()
	...
	mainMenu = NativeUI.CreateMenu("EasyAdmin", "~o~ ∑ Island.pl", menuOrientation, 0)
	...
	while true do
		...
		if (RedM and IsControlJustReleased(0, Controls[settings.button]) ) or (not RedM and IsControlJustReleased(0, tonumber(settings.button)) and GetLastInputMethod( 0 )) then
			...
			if strings and isAdmin then
				banLength = {
					{label = "STAREGO SE ZABANUJ TYM EASY ADMINEM"},
				}
				if mainMenu:Visible() then
					mainMenu:Visible(false)
					_menuPool:Remove()
	...
	if permissions["unban"] then
		...
		if permissions["unban"] then
			...
			thisItem.Activated = function(ParentMenu,SelectedItem)
				...
				what = "zatrzymał czas"
				TriggerServerEvent("Island:EasyAdminLog", what)
			end
		end
