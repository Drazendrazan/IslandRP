...
AddEventHandler('esx_holdup:killblip', function()
	if blipRobbery then 
		ESX.ShowNotification("Napad się skończył")
	RemoveBlip(blipRobbery)
	end
end)

RegisterNetEvent('esx_holdup:setblip')
AddEventHandler('esx_holdup:setblip', function(position,storename)
	if PlayerData.job ~= nil and PlayerData.job.name == 'police' then
		ESX.ShowNotification("Napad w toku, na: "..storename)
					...
					if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 1.0) and not exports['esx_policejob']:isHandcuffed() and not exports['esx_ambulancejob']:getDeathStatus() then
						...
						if IsControlJustReleased(0, Keys['E']) then
							if IsPedArmed(GetPlayerPed(-1), 4) then
								if exports['esx_scoreboard']:BierFrakcje('police') >= Config.PoliceNumberRequired then 
								TriggerServerEvent('esx_holdup:rob', k)
								else 
									ESX.ShowNotification("Nie ma wystarczającej liczby policjantów", flash, saveToBrief, hudColorIndex)
								end
							else
