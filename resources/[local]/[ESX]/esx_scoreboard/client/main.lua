...
function PlayerList()
	...
	if Id and PlayerData.job then
		local nui = {
			data = {},
			duties = 'Obywatele: <span style="color:deepskyblue">' .. players .. '</span> | SAST: ' .. ((police and police) > 0 and '<span style="color:green">Tak</span> (' .. (police < 3 and 'max 2' or 'min 3') .. ')' or '<span style="color:red">Nie</span>') .. " | SAMS: " .. ((ems and ems > 0) and '<span style="color:green">Tak</span> (' .. (ems < 4 and 'max 3' or 'min 4') .. ')' or '<span style="color:red">Nie</span>') .. " | TAXI: " .. ((taxi and taxi > 0) and '<span style="color:green">Tak</span> ' .. (taxi > 0 and '(min 1)' or '') or '<span style="color:red">Nie</span>') .. " | Mechanik: " .. ((mecano and mecano > 0) and '<span style="color:green">Tak</span> ' .. (mecano > 1 and '(min 2)' or '') or '<span style="color:red">Nie</span>') .." <br> Praca: "..PlayerData.job.label.."-"..PlayerData.job.grade_label
		}
...
AddEventHandler('esx_scoreboard:players', function(server)
	...
	for k,v in pairs(server) do		
		if v.id == Id then
			if v.group == 'best' or v.group == 'superadmin' or v.group == 'admin' or v.group == 'moderator' or v.group == 'support' or v.group == 'trailsupport' then
				IsAdmin = true
			else
				IsAdmin = false
			end
