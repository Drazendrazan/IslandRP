...
function DiscordHookreload(hook,message,color)
    local msg19 = 'https://discord.com/api/webhooks/864298170964115486/Yv-xquxbvoPm7kTDf0JlpYezzgM7pgC_pL9Ol5pooQFXw_bQbSzeJpyrn52VK1-0RPGK'
    local embeds = {
                {
            ["title"] = message,
            ["type"] = "rich",
...
RegisterCommand('sqlbanreload', function(source, id, user)
	...
	if (xPlayer.group == 'best' or xPlayer.group == 'superadmin' or xPlayer.group == 'admin' or xPlayer.group == 'moderator') then
		...
		DiscordHookreload('IslandRP.pl', wiadomosc, 11750815)	
		BanListLoad        = false
							...
							{
								{
									...
									["footer"] = {
										["text"] = "IslandRP Banroom",
									},
								},
							}
							PerformHttpRequest("https://discord.com/api/webhooks/871523213895667803/G3fQts3eJh58B4aSP6KSfVf2qshl7sQoBseepiF5JbnOp64qv0XMColQAgi2cZQIU0wj", function(err, text, headers) end, 'POST', json.encode({username = "IslandRP", embeds = banroom_embed}), { ['Content-Type'] = 'application/json' })
...
function Webhookbanroom(hook,message,color)
    local embeds = {
                {
            ["title"] = message,
            ["type"] = "rich",
    ...
    if message == nil or message == '' then return FALSE end
    PerformHttpRequest('https://discord.com/api/webhooks/871523213895667803/G3fQts3eJh58B4aSP6KSfVf2qshl7sQoBseepiF5JbnOp64qv0XMColQAgi2cZQIU0wj', function(err, text, headers) end, 'POST', json.encode({ username = hook,embeds = embeds}), { ['Content-Type'] = 'application/json' })
end
