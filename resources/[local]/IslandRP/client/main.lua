...
CreateThread(function()
	...
	while not IslandLoaded do
		if WorldName == 'default' or 'lspd' or 'alpha' or 'testowy' and "IslandRP" ~= IslandGlobal then
			Citizen.Wait(200)
			NetworkSetVoiceChannel(99999)
			Citizen.Wait(10)
			NetworkSetTalkerProximity(3.5 + 0.0)
			if getcall and "IslandRP" ~= IslandGlobal and WorldName == 'default' or 'lspd' or 'alpha' or 'testowy' then
...
CreateThread(function()
	while true do
		...
		AddTextEntryByHash('FE_THDR_GTAO', 'IslandRP.pl | GTA V')
		Citizen.Wait(60000)
	end
end)
...
RegisterNetEvent('Island:zacmienie')
AddEventHandler('Island:zacmienie', function(left)
	CreateThread(function()
		...
		PushScaleformMovieFunctionParameterString("UWAGA: Zaćmienie wyspy Island za " .. left .. " minut" .. (left == 1 and "ę" or (left < 5 and "y" or "")) .. "!")
...
RegisterNetEvent('Island:zacmienieanuluj')
AddEventHandler('Island:zacmienieanuluj', function(left)
	CreateThread(function()
		local scaleform = RequestScaleformMovie("breaking_news")
		while not HasScaleformMovieLoaded(scaleform) do
			Citizen.Wait(0)
		...
		SetClockTime(0, 00, 00)
		PushScaleformMovieFunction(scaleform, "SET_TEXT")
		PushScaleformMovieFunctionParameterString("UWAGA: Zaćmienie wyspy Island zostało anulowane!")
		PopScaleformMovieFunctionVoid()
...
RegisterNetEvent("Island:forceInTrunk")
AddEventHandler("Island:forceInTrunk", function(cop)
		...
		if not IsPedDeadOrDying(PlayerPedId(), 1) then
			...
			elseif IsThisModelACar(GetEntityModel(targetVehicle)) and checkTrunk(targetVehicle) and not DoesVehicleHaveDoor(targetVehicle, 6) and DoesVehicleHaveDoor(targetVehicle, 5) then
				...
				if DoesEntityExist(targetVehicle) then
					...
					ESX.TriggerServerCallback('Island_trunk:checkOccupied', function(plate)
						if plate then
							...
							TriggerServerEvent('Island_trunk:setUsed', true, inTrunk.plate)
						else
...
RegisterNetEvent("Island:forceOutTrunk")
AddEventHandler("Island:forceOutTrunk", function(cop)
	if inTrunk then
		local ped = PlayerPedId()
		ClearPedTasks(ped)
		DetachEntity(ped)
		...
		cameraTrunkDisable()
		TriggerServerEvent('Island_trunk:setUsed', false, inTrunk.plate)
		inTrunk = nil
	else
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'bagol',
		{
			...
			elseif data.current.value == 'go_in' then
				if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
					menu.close()
					TriggerEvent('Island:forceInTrunk', GetPlayerServerId(PlayerId()), vehicle)
				else
